from customDatasets.datasets import CustomImageDataset, CustomCollateFn
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(train_dataset,batch_size = batch_size, shuffle=True, pin_memory=True, collate_fn=CustomCollateFn)
    ...
    for epoch in tqdm( range(num_epochs), desc='Training', unit = 'Epoch', leave = False, disable=tqdm_disable):
        ...
        # Delete the last saved model if it exists, but not at epoch 0
        if epoch > 0:
            last_model_path = f'{save_location}model_{epoch}.pth'
