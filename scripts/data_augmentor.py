...
def transform_datapoint(image,label,identity_map=False,image_dim = 300):
    ...
    if identity_map:
        ...
        image_transform = transforms.Compose([
            ...
            transforms.ColorJitter(brightness=0.4, contrast=0.5, saturation=0.2, hue=0.2),
        ])
...
def convert_to_h5(image_dir, label_dir, database_file, image_shape = 300):
    ...
    with h5py.File(database_file, "w") as h5f:
        ...
        for i, file in tqdm(enumerate(filenames), total=len(filenames),desc='Converting to .h5 file', unit = ' Image'):
...
if __name__ == '__main__':
    ...
    for i in tqdm( range(0,len(data_files)), desc = 'Transforming dataset', unit = 'Image'):
